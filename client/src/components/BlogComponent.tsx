      ...
      {blogCards.map((blog) => (
        <div key={blog.id} className="bg-white rounded-lg shadow-lg overflow-hidden border border-gray-100 hover:shadow-xl transition-shadow duration-300">
          <div className="relative overflow-hidden">
            {blog.imageUrl ? (
              <img
                src={`${blog.imageUrl}?w=800&h=600&fit=fill&fm=webp&q=85`}
                alt={blog.title}
            ...
            <div className={`w-full h-56 bg-gradient-to-br from-gray-200 to-gray-300 flex items-center justify-center ${blog.imageUrl ? 'hidden' : ''}`}>
          ...
          <div className="p-6">
            <h3 className="text-xl font-bold text-gray-900 mb-2 leading-tight overflow-hidden" style={{ display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>{blog.title}</h3>
            {blog.summary && (
              <p className="text-sm text-gray-600 mb-4 leading-relaxed overflow-hidden" style={{ display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical' }}>{blog.summary}</p>
            )}
            <Link
              to={`/blog/${blog.id}`}
              className="inline-flex items-center px-4 py-2 text-sm font-medium rounded-lg bg-blue-600 text-white hover:bg-blue-700 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
            >
              Read more
