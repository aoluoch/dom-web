      ...
      {eventCards.map((event) => (
        <div key={event.id} className="bg-white rounded-lg shadow-lg overflow-hidden border border-gray-100 hover:shadow-xl transition-shadow duration-300">
          <div className="relative overflow-hidden">
            ...
            <div className={`w-full h-56 bg-gradient-to-br from-gray-200 to-gray-300 flex items-center justify-center ${event.imageUrl ? 'hidden' : ''}`}>
          ...
          <div className="p-6">
            <h3 className="text-xl font-bold text-gray-900 mb-2 leading-tight overflow-hidden" style={{ display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>{event.title}</h3>
            {event.summary && (
              <p className="text-sm text-gray-600 mb-4 leading-relaxed overflow-hidden" style={{ display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical' }}>{event.summary}</p>
            )}
            <Link
              to={`/events/${event.id}`}
