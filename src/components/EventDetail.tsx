...
const formatEventDate = (iso?: string): { weekday: string; full: string; time?: string } | undefined => {
  ...
  // Only surface a time when the source value actually carries one.
...
const EventDetail = () => {
  ...
  const view = useMemo(() => {
    ...
    const summary = getFirstLocaleString(entry.fields.eventSummary) ?? ""
    ...
    const dateIso = getFirstLocaleString(entry.fields.eventDate)
  ...
  const richTextOptions: Options = useMemo(
    () => ({
      renderMark: {
        ...
        [MARKS.CODE]: (text) => (
          <code className="rounded bg-gray-100 px-1.5 py-0.5 font-mono text-sm text-blue-700">{text}</code>
        ),
      },
      renderNode: {
        ...
        [BLOCKS.HEADING_3]: (_n, children) => (
          <h4 className="mt-6 mb-3 text-lg font-semibold text-gray-900">{children}</h4>
        ),
        [BLOCKS.HEADING_4]: (_n, children) => (
          <h5 className="mt-6 mb-2 text-base font-semibold text-gray-900">{children}</h5>
        ),
        ...
        [BLOCKS.OL_LIST]: (_n, children) => (
          <ol className="mb-6 list-inside list-decimal space-y-3 text-gray-700">{children}</ol>
        ),
  ...
  if (!view) {
    return (
      <div className="container mx-auto max-w-3xl px-4 py-20 text-center">
        ...
        <p className="mb-6 text-gray-600">The event you're looking for may have been moved or removed.</p>
        <Link
          to="/events"
          className="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-5 py-2.5 text-sm font-medium text-white hover:bg-blue-700"
        >
            ...
            <div className="overflow-hidden rounded-2xl bg-white shadow-xl ring-1 ring-gray-100">
              ...
              {view.descriptionDoc && (
                <div className="text-gray-700">
                  {documentToReactComponents(view.descriptionDoc, richTextOptions)}
                </div>
              )}
              </div>
            </div>
              ...
              <div className="rounded-2xl bg-white p-6 shadow-lg ring-1 ring-gray-100">
                ...
                {view.date ? (
                  <ul className="space-y-4">
                    <li className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="font-semibold text-gray-900">{view.date.full}</p>
                        <p className="text-sm text-gray-500">{view.date.weekday}</p>
                      </div>
                    </li>
                    {view.date.time && (
                      <li className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="font-semibold text-gray-900">{view.date.time}</p>
                        </div>
                      </li>
                    )}
                  </ul>
                  ...
                  <p className="text-sm text-gray-500">
                    Full event details are described in the section beside this panel.
                  </p>
                )}
